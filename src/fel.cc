...
namespace fel
{
    ...
    struct Rvalue
    {
        ...
        CalculateValue(const std::string& filename, Log* log) = 0;
    };
    ...
    struct AddRvalue : public Rvalue
    {
        ...
        AddRvalue(
                const Location&         lo,
                std::shared_ptr<Rvalue> l,
                std::shared_ptr<Rvalue> r)
            : location(lo), lhs(l), rhs(r)
        {}
        virtual ~AddRvalue() {}

        std::shared_ptr<Value>
        CalculateValue(const std::string& filename, Log* log)
        {
            ...
            if(lhs_value_int != nullptr)
            {
                ...
                if(rhs_value_int == nullptr)
                {
                    Add(log,
                        filename,
                        location,
                        Str() << "invalid operation for +: lhs is int but rhs is "
                              << rhs_value->TypeToString());
                    return nullptr;
                }
                return std::make_shared<IntValue>(
                        lhs_value_int->value + rhs_value_int->value);
            }
            ...
            if(lhs_value_string != nullptr)
            {
                ...
                if(rhs_value_string == nullptr)
                {
                    Add(log,
                        filename,
                        location,
                        Str() << "invalid operation for +: lhs is string but rhs is "
                              << rhs_value->TypeToString());
                    return nullptr;
                }
                return std::make_shared<StringValue>(
                        lhs_value_string->value + rhs_value_string->value);
            }

            Add(log,
                filename,
                location,
                Str() << "unknown operation for +: lhs is "
                      << lhs_value->TypeToString() << " but rhs is "
                      << rhs_value->TypeToString());
            return nullptr;
        }
    };
    ...
    struct ConstantRvalue : public Rvalue
    {
        ...
        std::shared_ptr<Value>
        CalculateValue(const std::string&, Log*) override
        {
            return value;
        }
    };
    ...
    ParseIntConstant(File* file, Log* log)
    {
        ...
        if(int_string == "")
        {
            Add(log,
                *file,
                "Failed to parse int, found " + CharToString(file->Peek()));
            return nullptr;
        }
        ...
        if(ss.fail())
        {
            Add(log,
                *file,
                "Internal error: Unable to get int from " + int_string);
            return nullptr;
        }
    ...
    ParseConstant(File* file, Log* log)
    {
        ...
        if(p == '\'' || p == '\"')
        {
            ...
            if(s == nullptr)
            {
                Add(log,
                    *file,
                    "Unable to parse string, found "
                            + CharToString(file->Peek()));
                return nullptr;
            }
...
#define EXPECT(cc)                                                             \
    ...
    {                                                                          \
        ...
        {                                                                      \
            Add(log,                                                           \
                *file,                                                         \
                "Error: Expected " #cc " but found " + CharToString(ec));      \
            return parsed;                                                     \
        }                                                                      \
    } while(false)
        ...
        while(file->HasMore())
        {
            ...
            if(IsFirstIdent(first_char))
            {
                ...
                if(ident == "")
                {
                    Add(log,
                        *file,
                        "internal parser error: expeced ident but got none");
                    return parsed;
                }
                if(false == SkipSpaces(file, log))
                {
            ...
            else
            {
                Add(log,
                    *file,
                    "unknown char found: " + CharToString(first_char));
                return parsed;
            }
        }
...
#undef EXPECT
    ...
    void
    Run(const Fel& fel, const Parsed& parsed, Log* log)
    {
    ...
    std::shared_ptr<Value>
    State::GetStack(int index) const
    {
        assert(index < 0);
    ...
    void
    Fel::SetFunction(const std::string& name, FunctionCallback callback)
    {
        functions[name] = callback;
    }

    void
    Fel::LoadAndRunString(
